...
void Trapeze::Print()
{
    std::cout << "Меньшая сторона = " << small_base << ", Большая сторона = " << big_base << ", Левая сторона = " <<  l_side << ", Правая сторона = " << r_side << ", square = " << this->getSquare() << ", type: трапеция" << std::endl;
}

std::ostream& operator <<(std::ostream &os, const Trapeze &obj)
{
    os << "(" << obj.small_base << " " << obj.big_base << " " << obj.l_side << " " << obj.r_side << ")," << "type: трапеция" << std::endl;
    return os;
}
